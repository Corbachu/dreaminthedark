...
static int S_LoadSfx(sfxdef_c *def, int downsmp, int adpcm, sfxhnd_t *handle)
{
    ...
    int iw, ix, iz, rate, sample, samples, datasize;
    byte *buf = NULL;
    ...
    if (memcmp(data, "RIFF", 4) == 0)
    {
        ...
        if (memcmp(wh->hdr2, "WAVEfmt ", 8)
            || wh->hdrsize != 16
            || (wh->format != 1 && wh->format != 0x20)
            || (wh->channels != 1 && wh->channels != 2)
            || (wh->bits != 16 && wh->bits != 8 && wh->bits != 4))
        {
            I_Printf("SFX Loader: WAV file unsupported format.\n");
            M_WarnError("SFX Loader: WAV file unsupported format.\n");
            delete[] data;
            return 0;
        }
        for (iz=0; iz<=56; iz++)
            if (memcmp(&data[iz], "data", 4) == 0)
                break;
        datasize = data[iz+4] | (data[iz+5] << 8) | (data[iz+6] << 16);
        iz += 8;
        ...
        if (wh->format == 0x20)
            ...
            samples = datasize / (wh->channels * wh->bits / 8);
        ...
        if (handle && buf)
        {
            ...
            {
                adpcm = 1;
                memcpy(buf, &data[iz], datasize);
            }
            else if (adpcm)
            {
                for (ix=0, iw=0; ix<(samples >> downsmp); ix+=2)
                {
                    int val;
                    if (wh->bits == 8)
                    {
            ...
            {
                for (ix=0, iw=0; ix<(samples >> downsmp); ix++)
                {
                    if (wh->bits == 8)
                    {
                        sample = sum_samples_8(&data[iz], downsmp + wh->channels - 1);
    ...
    if (buf)
    {
        ...
        fwrite(buf, 1, (samples >> downsmp) * 2 / (adpcm ? 4 : 1) + 2, out);
        fclose(out);
        delete[] buf;
    }
