...
static int16 width;
static int16 height;
static int16 mb_width;
static int16 mb_height;
static int16 mb_count;
static int16 vix;
static int16 vfc;
static uint16 *frames[2];
static uint16 *frame0;
...
extern uint8 pcm_buffer[]; // circular buffer of stereo pcm samples
...
#define CCIR_601
static uint16 __attribute__((aligned(16))) cb2x2[ROQ_CODEBOOK_SIZE][4];
static uint16 __attribute__((aligned(16))) cb4x4[ROQ_CODEBOOK_SIZE][16];

GLuint tex[1];
int texture_width, texture_height;
float scale_width, scale_height;

static uint32 movie_status = 0;
static kthread_t * movie_thread = NULL;

// define to accumulate and print timing info on movie player
#define PROFILE_MOVIE
#ifdef PROFILE_MOVIE
...
static void *do_movie(void *argp)
{
#ifdef PROFILE_MOVIE
    uint64 usec;
#endif
...
static int roq_unpack_quad_codebook(byte *buf, int size, int arg)
{
    ...
    for (i = 0; i < count2x2; i++)
    {
        ...
        for (j = 0; j < 4; j++)
        {
            ...
            cb2x2[i][j] = (uint16)(((r << RGB565_RED_SHIFT) & RGB565_RED_MASK) |
...
// decode RoQ frame
static int roq_unpack_vq(byte *buf, int size, uint16 arg)
{
...
static int decode_RVQ(byte *in, int chunk_size, uint16 chunk_arg, int chans)
{
    int i;
    ...
    int16 *cb = (int16 *)in;
    int16 *out = (int16 *)&pcm_buffer[awix];
...
static int32_t decode_RoQ(uint8_t *in, int32_t size)
{
    uint16 chunk_id, chunk_arg;
    uint32 chunk_size;
    int status = ROQ_SUCCESS;
#ifdef PROFILE_MOVIE
    uint64 usec;
    ...
    while (size >= CHUNK_HEADER_SIZE && status >= ROQ_SUCCESS)
    {
        ...
        case RoQ_QUAD_INFO:
            ...
            frame0 = (uint16 *)malloc(texture_width * height * 2);
            frame1 = (uint16 *)malloc(texture_width * height * 2);
            if (!frame0 || !frame1)
        ...
        case RoQ_MONO_VQ:
            ...
            {
                // setup stream
                stream_hnd = snd_stream_alloc(NULL, PCM_BUF_SIZE);
                if(stream_hnd == SND_STREAM_INVALID)
                {
...
static int get_chunk(epi::file_c *F, int *length)
{
    ...
    uint32 chunk_size;
#ifdef PROFILE_MOVIE
    uint64 usec = timer_us_gettime64();
#endif
...
int I_PlayMovie(char *name)
{
    ...
    if (name)
    {
        ...
        movie_status = 0;
        movie_thread = thd_create(0, do_movie, NULL);
        if(!movie_thread)
        {
